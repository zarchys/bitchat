...
final class GeoRelayDirectory {
    ...
    private func fetchRemote() {
        ...
        Task.detached {
            ...
            if !ready {
                SecureLogger.log("GeoRelayDirectory: Tor not ready; skipping remote fetch (fail-closed)", category: .session, level: .warning)
                return
            }
                ...
                if let data = data, error == nil, let text = String(data: data, encoding: .utf8) {
                    ...
                    if !parsed.isEmpty {
                        Task { @MainActor in
                            ...
                            SecureLogger.log("GeoRelayDirectory: refreshed \(parsed.count) relays from remote", category: .session, level: .info)
                        }
                        return
                    }
                }
                SecureLogger.log("GeoRelayDirectory: remote fetch failed; keeping local entries", category: .session, level: .warning)
            }
            task.resume()
        }
    }
    ...
    private func persistCache(_ text: String) {
        ...
        do {
            ...
            SecureLogger.log("GeoRelayDirectory: failed to write cache: \(error)", category: .session, level: .warning)
        }
    }
    ...
    private func loadLocalEntries() -> [Entry] {
        ...
        SecureLogger.log("GeoRelayDirectory: no local CSV found; entries empty", category: .session, level: .warning)
        return []
    }
