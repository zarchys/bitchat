...
final class TorManager: ObservableObject {
    ...
    private func startTorViaEmbedAPI() -> Bool {
        ...
        if tor_host_is_running() != 0 {
            SecureLogger.log("TorManager: embed reports already running", category: .session, level: .info)
            return true
        }
        dir.withCString { dptr in
            socks.withCString { sptr in
                control.withCString { cptr in
                    ...
                    started = (rc == 0)
                    if rc != 0 {
                        SecureLogger.log("TorManager: tor_host_start failed rc=\(rc)", category: .session, level: .error)
                    } else {
                        SecureLogger.log("TorManager: tor_host_start OK (\(socks), control \(control))", category: .session, level: .info)
                    }
                }
            }
        }
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            await MainActor.run {
                ...
                if ready {
                    SecureLogger.log("TorManager: SOCKS ready at \(self.socksHost):\(self.socksPort) [embed]", category: .session, level: .info)
                } else {
                    ...
                    SecureLogger.log("TorManager: SOCKS not reachable (timeout) [embed]", category: .session, level: .error)
                }
            }
        }
        return true
    ...
    private func startTorViaDlopen() -> Bool {
        guard let fwURL = frameworkBinaryURL() else {
            SecureLogger.log("TorManager: no embedded tor framework found", category: .session, level: .warning)
            return false
        }

        // Load the library
        let mode = RTLD_NOW | RTLD_LOCAL
        SecureLogger.log("TorManager: dlopen(\(fwURL.lastPathComponent))…", category: .session, level: .info)
        ...
        SecureLogger.log("TorManager: launching tor_main with torrc", category: .session, level: .info)
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            await MainActor.run {
                ...
                if !ready {
                    ...
                    SecureLogger.log("TorManager: SOCKS not reachable (timeout)", category: .session, level: .error)
                } else {
                    SecureLogger.log("TorManager: SOCKS ready at \(self.socksHost):\(self.socksPort)", category: .session, level: .info)
                }
    ...
    private func startTorViaLinkedSymbol() -> Bool {
        ...
        SecureLogger.log("TorManager: starting tor_main (static)", category: .session, level: .info)
        let argc = Int32(argv.count)
        DispatchQueue.global(qos: .utility).async {
            // Build stable C argv in this thread
            let cStrings: [UnsafeMutablePointer<CChar>?] = argv.map { strdup($0) }
